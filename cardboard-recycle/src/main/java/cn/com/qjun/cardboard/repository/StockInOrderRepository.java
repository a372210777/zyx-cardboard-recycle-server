...
public interface StockInOrderRepository extends JpaRepository<StockInOrder, String>, JpaSpecificationExecutor<StockInOrder> {
    ...
    @Query(value = "select max(right(id,6)) from biz_stock_in_order where date(stock_in_time) = ?", nativeQuery = true)
    ...
    @Query(value = "select * from biz_stock_in_order o  join biz_stock_in_order_item oi on o.id = oi.stock_in_order_id \n" +
            " join basic_material m on oi.material_id = m.id  where o.stock_in_time between ?1 and ?2 and o.deleted = 0 order by o.stock_in_time asc", nativeQuery = true)
    List<StockInOrder> findAllByOrderByStockInTimeDesc(String beginDate, String endDate);

    @Query(value = "select * from biz_stock_in_order o  left join biz_stock_in_order_item oi on o.id = oi.stock_in_order_id \n" +
            " left join basic_material m on oi.material_id = m.id  where o.id= ?  order by o.stock_in_time asc", nativeQuery = true)
    List<StockInOrder> selectOneById(String id);
